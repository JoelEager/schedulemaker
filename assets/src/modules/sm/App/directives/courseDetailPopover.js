angular.module("sm").directive("courseDetailPopover", function($http, $filter) {
    var RMPUrl = $filter("RMPUrl"), parseTimes = $filter("parseSectionTimes"), formatTime = $filter("formatTime");
    ...
    return {
        ...
        link: function(scope, elm) {
            if (scope.sectionId != "") {
                var loaded = false, opened = false, $body = $("body");
                ...
                elm.on("click", function() {
                    if (!loaded) {
                        loaded = true;
                        $http.post("/entity/courseForSection", $.param({
                            id: scope.sectionId
