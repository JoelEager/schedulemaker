    ...
    function scheduleActions(scope, elm) {
        ...
        scope.scheduleActions = {
            save: function(saveType) {
                if (saveType == "create") {
                    ...
                    getSavedInfo().then(function(data) {
                        scope.notification = "This schedule can be accessed at " + '<a href="' + data.url + '" target="_blank">' + data.url + "</a><br><em>This schedule will be removed" + " after 3 months of inactivity</em>";
                    }, function(error) {
                        console.log(error);
            ...
            downloadImage: function($event) {
                ...
                getSavedInfo().then(function(data) {
                    popup.location = "http://" + window.location.hostname + "/img/schedules/" + parseInt(data.id, 16) + ".png";
                });
            },
            print: function() {
                ga("send", "event", "schedule", "print");
                var reloadSchedule = angular.copy(scope.state.drawOptions);
                reloadSchedule.term = scope.state.requestOptions.term, reloadSchedule.courses = scope.schedule;
