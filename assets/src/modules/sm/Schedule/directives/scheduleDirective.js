angular.module("sm").directive("schedule", function($timeout, $filter) {
    ...
    Schedule.prototype.init = function(options) {
        ...
        if (!this.drawOptions.startTime && this.drawOptions.startTime !== 0 || !this.drawOptions.endTime) {
            return false;
        }
    ...
    Schedule.prototype.drawGrid = function() {
        ...
        // Set up grid
        var rawHeight = hourArray.length * 40, globalOpts = {
            height: rawHeight + 25,
            hoursWidth: 5
        }, rawDayWidth = 100 / numDays, dayPadding = 1, dayOpts = {
            num: numDays,
    ...
    Schedule.prototype.drawCourse = function(course, index) {
        ...
        for (var t = 0; t < course.times.length; t++) {
            ...
            var courseStart = time.start, courseEnd = time.end, shorten = 0;
            ...
            if (courseStart < startTime || courseStart > endTime || courseEnd > endTime) {
                ...
                if (courseStart < startTime && courseEnd > startTime) {
                    ...
                    if (courseEnd > endTime && courseStart < endTime) {
                        courseEnd = endTime;
                        shorten = 1;
                    } else {
            ...
            if (course.courseNum != "non") {
                var location = (this.drawOptions.bldgStyle == "code" ? time.bldg.code : time.bldg.number) + "-" + time.room, instructor = course.instructor, courseNum = course.courseNum;
            } else {
                var location = "", instructor = "", courseNum = "";
            }
    ...
    Schedule.prototype.drawCourses = function() {
        ...
        for (var coursesIndex = 0, coursesLength = this.scope.schedule.length; coursesIndex < coursesLength; coursesIndex++) {
            ...
            if (course.online && !course.hasOwnProperty("times")) {
                ...
                if (course.hasOwnProperty("times")) {
        ...
        link: {
            pre: function(scope, elm, attrs) {
                ...
                scope.itemEnter = function($event) {
                    var $target = $($event.target), $scope = $target.scope();
            ...
            post: function(scope, elm) {
                var update = function(options) {
                    if (scope.scheduleController.init(options)) {
                        ...
                        $timeout(function() {
                            var offset = elm.find("svg").offset(), vert = 1 - parseFloat("0." + ("" + offset.top).split(".")[1]);
